library ListGroupsCommand initializer init requires Commands, ChatSystem, StringUtil, CSAPI
    struct ListGroupsCommand extends Command
        ...
        public method execute takes player initiator, integer argc returns nothing
            ...
            call CSAPI_sendSystemMessageToPlayer(initiator, "|cff808000Players in group |cff0080C0" + chatGroup.name + "|r:")
            loop
                exitwhen i == 0
                set currentProfile = groupMembers[i]
                call CSAPI_sendSystemMessageToPlayer(initiator, currentProfile.name)
                set i = i - 1
            endloop
        endmethod
    endstruct
