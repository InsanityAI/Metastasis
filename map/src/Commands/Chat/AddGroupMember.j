library AddGroupMemberCommand initializer init requires Commands, ChatSystem, ChatGroups, ChatProfiles, StringUtil, Anonymity, CSAPI
    struct AddGroupMemberCommand extends Command
        ...
        public method execute takes player initiator, integer argc returns nothing
            ...
            if chatGroup.contains(targettedProfile) then
                call CSAPI_sendSystemMessageToPlayer(initiator, "|cFFFF0000Error: Player is already in this group!")
                return
            endif

            call chatGroup.add(targettedProfile)
            call CSAPI_sendSystemMessageToGroup(chatGroup, "|cFF00FFFFNotice:|r " + targettedProfile.name + " has been added to this group.")
        endmethod
    endstruct
