library StateGrid initializer Init requires StringWidth, PlayerColor, Timeout, Table, PlayerUtil 
    ...
    public function SetPlayerName takes player thisPlayer, string name returns nothing 
        local PlayerData data = getPlayerData(thisPlayer) 
        call MultiboardSetItemValue(items[data.rowIndex * 3 + 0], PlayerColor_GetByPlayer(thisPlayer).color + StringWidth_substringTextToMultiboardWidth(name, 0, nameWidth)) 
    endfunction 
    ...
    private function initForPlayer takes player enumPlayer returns nothing 
        ...
        call MultiboardSetItemValue(items[3 * currentIndex + 0], PlayerColor_GetByPlayer(enumPlayer).color + StringWidth_substringTextToMultiboardWidth(GetPlayerName(enumPlayer), 0, nameWidth)) 
