library ChatUI initializer init requires Table, Timeout
    globals 
        ...
        private string CHAT_FONT = "Fonts\\BLQ55Web.ttf" //font used by messages(default wc3 chat font is "Fonts\\BLQ55Web.ttf")                          
    ...
    private struct MessageFrame 
        ...
        public method setContent takes string timestamp, string msgType, string text, string icon returns nothing 
            call BlzFrameSetText(this.frameMessageTimeStamp, timestamp) 
            call BlzFrameSetText(this.frameMessageType, msgType) 
    ...
    private function showNewMessage takes string timestamp, ChatProfile from, string message, string messageType returns nothing 
        ...
        call newMessageFrame.setContent(timestamp, messageType, message, from.icon) 
