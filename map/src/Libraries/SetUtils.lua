...
do
    ...
    local loopId = 0 -- an Id unique per iteration currently running on any Set. This is used to retreive all the other information. Recycled after a loop has finished running. Counts upwards.
    local loopSets = {} ---@type table<integer,Set> memorizes the Set that is iterated through for a given loopId
    local loopLastElements = {} ---@type table<integer,any> memorizes the last Set element that was looped over for a given loopId
    local loopCounters = {} ---@type table<integer,integer> memorizes the last array position that was looped over for a given loopId
    ...
    ---@param loopId integer loopId to identify one particular running iteration.
    ---@return any element
    local function iterator(loopId)
        local loopSet = loopSets[loopId]
        local i = loopCounters[loopId]
        if loopLastElements[loopId] == loopSet.orderedKeys[i] then --check, if the last iterated key is still in place. If not, it has been removed in the last part of the iteration.
            loopCounters[loopId] = loopCounters[loopId] +
            1                                                      --only increase counter, if the last element in loop is still part of the Set. Otherwise another element has been downshifted to this position, so we need to use same counter again.
            i = i + 1
        end
        loopLastElements[loopId] = loopSet.orderedKeys[i]
        return loopLastElements[loopId]
    end
    ...
    function Set:elements()
        ...
        while loopLastElements[loopId] ~= nil do
            loopId = loopId + 1
        end
        loopSets[loopId] = self
        loopCounters[loopId] = 1
        return iterator, loopId
    end
    ...
    function SetUtils.getUnitsInRectMatching(whichRect, condition)
        ...
        return enumGroupToSet(GroupEnumUnitsInRect, converted_yn and convertedCondition or nil, whichRect,
            convertedCondition)
    end
    ...
    function SetUtils.getUnitsInRangeMatching(x, y, radius, condition)
        local convertedCondition, converted_yn = conditionIfNecessary(condition)
        return enumGroupToSet(GroupEnumUnitsInRange, converted_yn and convertedCondition or nil, x, y, radius,
            convertedCondition)
    end
    ...
    function SetUtils.getUnitsOfPlayerMatching(whichPlayer, condition)
        local convertedCondition, converted_yn = conditionIfNecessary(condition)
        return enumGroupToSet(GroupEnumUnitsOfPlayer, converted_yn and convertedCondition or nil, whichPlayer,
            convertedCondition)
    end
    ...
    function SetUtils.getUnitsSelected(whichPlayer, condition)
        ...
        local convertedCondition, converted_yn = conditionIfNecessary(condition)
        return enumGroupToSet(GroupEnumUnitsSelected, converted_yn and convertedCondition or nil, whichPlayer,
            convertedCondition)
    end
    ...
    function SetUtils.createTriggers()
        ...
        if CUSTOM_DEFEND_ABICODE then
            ...
            hasUnitBeenRemovedCondition = Condition(function() return (GetIssuedOrderId() == 852056) and
                GetUnitAbilityLevel(GetTriggerUnit(), CUSTOM_DEFEND_ABICODE) == 0 end)                                                                                          --undefend order. This one is issued upon units leaving the game, but also under other circumstances. Ability-Level == 0 proves the removed from the game event.
