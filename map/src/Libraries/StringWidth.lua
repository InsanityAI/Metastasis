if not Debug then -- initialize if DebugUtils is not enabled
    ------------------------
    ----| String Width |----
    ------------------------
    ...
    do
        ...
        -- ---@param multichar string
        -- ---@param textLanguage? '"ger"' | '"eng"'
        -- ---@param width number
        -- function string.substringTextToMultiboardWidth(multichar, textLanguage, width)
        --     local stringtable = table.pack(multichar:byte(1, -1))
        --     local totalWidth = 0.
        --     for i = 1, stringtable.n do
        --         totalWidth = totalWidth + string.charMultiboardWidth(stringtable[i], textLanguage)
        --         if totalWidth > width then
        --             return string.sub(multichar, 0, i - 1)
        --         end
        --     end
        --     return multichar
        -- end
