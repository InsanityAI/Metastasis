...
OnInit.module("TimerQueue", function()
    --[[------------------------------------------------------------------------------------------------------------------------------------------------------------
*
*    --------------------------------
...
---------------------------------------------------------------------------------------------------------------------------------------------------------]]
    ---@class TimerQueueTask
    ---@field timeout number

    ---@class TimerQueueElement: TimerQueueTask
    ---@field timeout number
    ---@field next TimerQueueElement
    ---@field callback fun()
    ---@field n integer
    ...
    local unpack, max, timerStart, timerGetElapsed, pauseTimer = table.unpack, math.max, TimerStart, TimerGetElapsed,
        PauseTimer
    ...
    ---@param timerQueue TimerQueue
    ---@param task TimerQueueElement
    local function enqueueTask(timerQueue, task)
        local timeout = math.max(task.timeout, 0.)
        local queue = timerQueue.queue
        timerQueue.n = timerQueue.n + 1
        -- Sort timeouts in descending order
        local current = queue
        local current_timeout = current.next.timeout -
            max(timerGetElapsed(timerQueue.timer), 0.)                -- don't use TimerGetRemaining to prevent bugs for expired and previously paused timers.
        ...
        if current == queue then                                   --New callback is the next to expire
            task.next.timeout = max(current_timeout - timeout, 0.) --adapt element that was previously on top. Subtract new timeout and subtract timer elapsed time to get new timeout.
            timerStart(timerQueue.timer, timeout, false, timerQueue.on_expire)
            if timerQueue.paused then
                timerQueue:pause()
            end
        else
            task.next.timeout = max(task.next.timeout - timeout, 0.) --current.next might be the root element (queue), so prevent that from dropping below 0. (although it doesn't really matter)
        end
    end
    ...
    function TimerQueue:callDelayed(timeout, callback, ...)
        local task = TimerQueueElement.create(timeout, callback, ...)
        enqueueTask(self, task)
        return task
    end
    ...
    ---@param task TimerQueueTask|TimerQueueElement
    function TimerQueue:cancel(task)
        local queue = self.queue
        local current = queue

        local current_timeout = current.next.timeout - max(timerGetElapsed(self.timer), 0.)
        -- find previous node of task to be cancelled
        while current.next ~= queue and current.next ~= task do
            current = current.next
            current_timeout = current.next.timeout
        end

        -- if task's list position is found
        if current.next == task then
            self.n = self.n - 1
            current.next = task.next -- remove task from list

            -- if there is no element in list, stop timer and do nothing
            if self.n == 0 then
                local timer = self.timer
                -- These two functions below may not be necessary
                timerStart(timer, 0, false, nil) --don't put in on_expire as handlerFunc, because it can still expire and reduce n to a value < 0.
                pauseTimer(timer)
            elseif queue == current then         -- if it's element that is next to be executed, reduce by current timer runtime and add removed tasks's timeout
                current.next.timeout = current.next.timeout - current_timeout + task.timeout
            elseif queue ~= current.next then    -- add timeout to next element only if there is a next element
                current.next.timeout = current.next.timeout + task.timeout
            end
        end
    end

    ---@param task TimerQueueTask
    ---@param timeout number
    function TimerQueue:setTaskTimeout(task, timeout)
        TimerQueue:cancel(task)
        task.timeout = timeout
        enqueueTask(self, task --[[@as TimerQueueElement]])
    end
